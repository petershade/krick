...
static int	ft_pos_prec_x(t_flags *flags, int len_x, int len, char *str)
{
...
static int	ft_neg_prec_x(t_flags *flags, int len_x, int len, char *str)
{
...
int	print_x(unsigned int num, t_flags *flags)
{
	...
	if (flags->precision == -1)
		len += ft_neg_prec_x(flags, len_x, len, str);
	if (flags->precision >= 0)
		len += ft_pos_prec_x(flags, len_x, len, str);
	free(str);
	return (len + len_x);
}
